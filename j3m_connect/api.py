...
class ApiClient(QObject):
    ...
    def _finished(self):
        ...
        if not message:
            if status in (401, 403):
                ...
                message = (
                    "Redirecionamento recusado. Contate o responsável "
                    "pela configuração do plugin."
                )
            elif (
                status is not None
                ...
                message = (
                    "API indisponível ou falha de rede/TLS. "
        ...
        if not message and status != 204 and not no_data:
            try:
                ...
                if self._contains_secret(payload):
                    message = (
                        "Resposta recusada por conter a credencial de acesso."
                    )
                    payload = None
    ...
    def _contains_secret(self, value):
        if isinstance(value, str):
            return bool(
                self._secret
                and self._secret in value
            )

        if isinstance(value, dict):
            return any(
                self._contains_secret(k)
                or self._contains_secret(v)
                for k, v in value.items()
            )

        if isinstance(value, list):
            return any(
                self._contains_secret(item)
                for item in value
            )

        return bool(
            self._secret
            and self._secret in str(value)
        )